...
export default function WaterDeficit_p(Irrigation, Kc, ETcp, availableWater, Weather, plantDate) {
  ...
  deficitVector[0] = ETcp.ETcp[0];
  for(i = 1; i < ETcp.ETcp.length; i++){
    deficitVector[i] = Math.min(Math.max(deficitVector[i-1] - (Irrigation.Amount[i] + precipitation[i].rain) + ETcp.ETcp[i], 0), availableWater.Taw[i]);
  }
